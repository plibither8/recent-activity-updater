...
module.exports = async function () {
  ...
  const SPOTIFY_ACCESS_TOKEN = await fetch(
  ...
  const LASTFM_API_TOP_ARTISTS =
    LASTFM_API_BASE + "&method=user.gettopartists&period=7day";
  ...
  for (let i = 0; i < 5; i++) {
    const { name, playcount, url } = data.topartists.artist[i];
  ...
  const LASTFM_API_WEEKLY_TRACKS =
    LASTFM_API_BASE + "&method=user.getweeklytrackchart";
  let weeklyTrackList = await fetch(LASTFM_API_WEEKLY_TRACKS).then((res) =>
    res.json()
  );
